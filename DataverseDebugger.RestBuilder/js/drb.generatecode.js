 ...
 * Legacy parameters are kept so existing generation methods do not need refactoring,
 * but only Fetch API and FetchXML editors are rendered now.
 * @param {string[]} codeXrmWebApi (legacy, unused)
 * @param {string[]} codeXrmWebApiExecute (legacy, unused)
 * @param {string[]} codejQuery (legacy, unused)
 * @param {string[]} codeXMLHttpRequest (legacy, unused)
 * @param {string[]} codeFetchAPI Code Fetch API
 * @param {string[]} codePortals (legacy, unused)
 * @param {string[]} codeFetchXML Code FetchXML
 * @param {string[]} codeSDK (legacy, unused)
 */
DRB.GenerateCode.SetCodeEditors = function (codeXrmWebApi, codeXrmWebApiExecute, codejQuery, codeXMLHttpRequest, codeFetchAPI, codePortals, codeFetchXML, codeSDK) {
    ...
    if (DRB.Utilities.HasValue(DRB.Settings.Editors["code_fetchapi"])) {
        DRB.Settings.Editors["code_fetchapi"].session.setValue(codeFetchAPI.join('\n'));
    }
    if (DRB.Utilities.HasValue(DRB.Settings.Editors["code_fetchxml"])) {
...
\tOptions = __options__,`;
            ...
            if (optionSetTypes.indexOf(column.AttributeType) > -1) {
                ...
                if (DRB.Utilities.HasValue(DRB.Settings.Editors["code_powerquery2"])) {
...
DRB.GenerateCode.Grid = function (requestType) {
    if (requestType !== "retrievesingle" && requestType !== "retrievemultiple") { return; }
    if ($("#" + DRB.DOM.Grid.Div.Id).length === 0) { return; }
