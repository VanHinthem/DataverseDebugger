...
DRB.UI.DisplayDialog = function (title, message, className, size, okCallBack, askQuestion, confirmLabel, cancelLabel) {
    bootbox.hideAll();
    var dialogClass = "drb-dialog";
    if (className === "btn-danger") { dialogClass += " drb-dialog--danger"; }
    else { dialogClass += " drb-dialog--primary"; }

    var dialogMessage = message;
    if (!DRB.Utilities.HasValue(dialogMessage)) { dialogMessage = ""; }

    var isStringMessage = typeof dialogMessage === "string" || typeof dialogMessage === "number";
    if (isStringMessage === true) {
        dialogMessage = dialogMessage.toString();
        if (dialogMessage.indexOf("drb-dialog__") === -1) {
            dialogMessage = '<div class="drb-dialog__message">' + dialogMessage + '</div>';
        }
    } else {
        var $dialogMessage = null;
        if (typeof window !== "undefined" && window.jQuery && dialogMessage instanceof window.jQuery) {
            $dialogMessage = dialogMessage;
        } else if (dialogMessage instanceof HTMLElement) {
            $dialogMessage = $(dialogMessage);
        }

        if ($dialogMessage !== null) {
            if ($dialogMessage.hasClass("drb-dialog__message") === false) {
                dialogMessage = $("<div>", { class: "drb-dialog__message" }).append($dialogMessage);
            } else {
                dialogMessage = $dialogMessage;
            }
        } else {
    ...
    var properties = {
        ...
        buttons: { ok: { label: "OK", className: className } }
    };
...
DRB.UI.Show = function (title, message, size, okCallBack) {
    DRB.UI.DisplayDialog(title, message, "btn-primary", size, okCallBack);
}
...
DRB.UI.ShowError = function (title, message, size, okCallBack) {
    DRB.UI.DisplayDialog(title, message, "btn-danger", size, okCallBack);
}
...
DRB.UI.ShowLoading = function (message, size) {
    ...
    if (DRB.Utilities.HasValue(message) && message.indexOf("<br") > -1) {
        var parts = message.split(/<br\s*\/?\s*>/i);
        if (parts.length > 0) {
            title = parts[0];
            subtitle = parts.slice(1).join("<br />");
        }
    }

    var loadingMessage = '<div class="drb-dialog__loading">' +
        '<p class="drb-dialog__title">' + title + '</p>';

    if (DRB.Utilities.HasValue(subtitle)) {
        loadingMessage += '<p class="drb-dialog__subtitle">' + subtitle + '</p>';
    }

    loadingMessage += '<div class="drb-progress"><span class="drb-progress__bar"></span></div>' +
        '</div>';
...
DRB.UI.ShowQuestion = function (title, message, size, comfirmCallBack) {
    DRB.UI.DisplayDialog(title, message, "btn-danger", size, comfirmCallBack, true);
}
...
DRB.UI.ShowExport = function (title, message, size, comfirmCallBack) {
    DRB.UI.DisplayDialog(title, message, "btn-primary", size, comfirmCallBack, true, "Export", "Cancel");
}
